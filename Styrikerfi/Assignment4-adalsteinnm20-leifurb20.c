...
void *my_malloc(uint64_t size) {
    ...
    while (_firstFreeBlock != NULL) {
        if (_firstFreeBlock->size > size){
            Block *new = (Block*)((char*)_firstFreeBlock + size);

            new->size = (_firstFreeBlock->size) - size;
            _firstFreeBlock->size = new->size;

            uint8_t *res = _firstFreeBlock->data;
            return res;
        }
        if (_firstFreeBlock->size == size) {
            uint8_t * res = (_firstFreeBlock->data);
...
void my_free(void *address)
{
    ...
    while (_firstFreeBlock->next != NULL){
        if (_firstFreeBlock->data == block->data){

            _firstFreeBlock->size = _firstFreeBlock + _firstFreeBlock->next->size;
            _firstFreeBlock = _firstFreeBlock->next;
        }
        else{
            _firstFreeBlock = _firstFreeBlock->next;
